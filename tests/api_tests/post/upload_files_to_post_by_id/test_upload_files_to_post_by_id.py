...
class TestUploadFilesToPostById:
    ...
    def test_upload_files_successful(self, post_steps, create_and_delete_post_fixture):
        ...
        post_steps.upload_files_successful(payload, create_and_delete_post_fixture.data.id)

    @pytest.mark.high
    @allure.title('Fail to upload files without authorization')
    def test_upload_files_without_auth(self, post_steps, create_and_delete_post_fixture):
        test_file = get_test_file_path("test_image.jpg")
        payload = UploadFilePayload(
            files=[FileItem(path=test_file, file_name="test_image.jpg", mime_type="image/jpeg")])
        post_steps.upload_files_without_auth(payload, create_and_delete_post_fixture.data.id)
