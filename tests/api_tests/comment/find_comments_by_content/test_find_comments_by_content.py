...
class TestFindCommentsByContent:
    ...
    def test_find_comments_by_content_positive(self, comment_steps, create_and_delete_post):
        payload = CreateCommentPayload(objectId=create_and_delete_post.data.id)
        comment_steps.create_comment_successful(payload)
        comment_steps.find_comments_by_content_successful(create_and_delete_post.data.content.id)

    @pytest.mark.high
    @allure.title('Find comments by content without auth')
    def test_find_comments_by_content_without_auth(self, comment_steps, create_and_delete_post):
        payload = CreateCommentPayload(objectId=create_and_delete_post.data.id)
        comment_steps.create_comment_successful(payload)
        comment_steps.find_comments_by_content_without_auth(create_and_delete_post.data.content.id)
